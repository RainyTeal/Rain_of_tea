# -*- coding: utf-8 -*-
"""
SQLite 数据库增删改查工具（自动发现表）
========================================
功能：
  1. 连接任意 SQLite 数据库（文件不存在时自动创建）
  2. 自动发现数据库中所有数据表，无需硬编码表名
  3. 自动读取表结构（列名、类型、主键），按表结构提示输入
  4. 增（insert）/ 删（delete）/ 改（update）/ 查（select，含条件查询）
...
依赖：仅使用 Python 标准库 sqlite3，无需安装任何第三方包。
"""
